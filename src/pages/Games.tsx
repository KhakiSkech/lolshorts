...
export function Games() {
  ...
  const [gamesData, setGamesData] = useState<Map<string, GameMetadata>>(
    new Map(),
  );
  ...
  const getResultClass = (result: string | null) => {
    if (result === "Win")
      return "text-gaming-cyan border-gaming-cyan/40 bg-gaming-cyan/10";
    if (result === "Loss")
      return "text-gaming-magenta border-gaming-magenta/40 bg-gaming-magenta/10";
      ...
      <div className="space-y-6">
        ...
        <div className="space-y-4">
          {[1, 2, 3].map((i) => (
            <div
              key={i}
              className="bg-black/40 rounded-lg border border-white/5 p-4"
            >
    ...
    <div className={pageStyles.container}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        <div className="bg-black/40 rounded-lg border border-white/5 p-4">
          ...
          <p className="text-3xl font-bold text-gaming-cyan mt-1">
            {stats.total_games}
          </p>
        </div>
        <div className="bg-black/40 rounded-lg border border-white/5 p-4">
          <p className="text-sm text-muted-foreground">
            {t("games.stats.totalClips")}
          </p>
          <p className="text-3xl font-bold text-gaming-cyan mt-1">
            {stats.total_clips}
          </p>
      ...
      {error && (
        <div className="p-4 mb-6 bg-gaming-magenta/10 border border-gaming-magenta/30 rounded-lg">
          <p className="text-sm text-gaming-magenta">
              ...
              <Select value={filterMode} onValueChange={setFilterMode}>
                ...
                <SelectContent>
                  <SelectItem value="all">
                    {t("games.filter.allModes")}
                  </SelectItem>
                  {Array.from(
                    new Set(
                      Array.from(gamesData.values()).map((g) => g.game_mode),
                    ),
                  ).map((mode) => (
                  ...
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        {gameMetadata.result && (
                          <span
                            className={`text-xs font-bold px-2 py-0.5 rounded border ${getResultClass(gameMetadata.result)}`}
                          >
                    ...
                    <div className="flex gap-2">
                      ...
                      <Button
                        ...
                        className="min-h-[44px]"
                        onClick={() =>
                          handleCreateHighlight(gameMetadata.game_id)
                        }
                  ...
                  <div>
                    ...
                    <p className="font-medium">
                      {gameMetadata.end_time
                        ? formatDuration(
                            (new Date(gameMetadata.end_time).getTime() -
                              new Date(gameMetadata.start_time).getTime()) /
                              1000,
                          )
                    ...
                    >
                      {gameMetadata.end_time
                        ? t("games.game.complete")
                        : t("games.game.inProgress")}
                    </Badge>
                  </div>
