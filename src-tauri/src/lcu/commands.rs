...
// Note: ALL LCU commands are FREE tier features - no authentication required
// Game detection and monitoring should be available to all users

// Global LCU client (initialized lazily)
static LCU_CLIENT: Lazy<Arc<Mutex<LcuClient>>> =
...
pub async fn get_current_game(state: State<'_, AppState>) -> Result<Option<GameInfo>, String> {
    ...
    client.get_current_game().await.map_err(|e| e.to_string())
}
